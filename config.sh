...
KV_CACHE KV_CACHE_SIZE COT_TASK COT_STEPS COT_LR COT_SFT COT_SFT_STEPS COT_SFT_LR COT_INIT COT_GROUP COT_BF16 COT_SAMPLES_EVERY COT_KL_COEF COT_MAX_NEW_TOKENS COT_FLAGS \
...
COT_BF16="${COT_BF16:-1}"           # 1 = bfloat16 live weights for both CoT sub-stages.
                                    # Halves the weights, the activations and the frozen
                                    # reference; the CHECKPOINT is still written in fp32, and
                                    # the log-probs are still computed in fp32. 0 = fp32
                                    # throughout.
...
[ "$COT_BF16" = "0" ]    && COT_FLAGS="$COT_FLAGS --no-cot_bf16"
