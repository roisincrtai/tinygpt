"""
rlhf.py -- stage 8's logic: RLHF instruction tuning.
...
Assembles the three models the recipe needs and hands them to the optimiser in ppo.py:
    ...
    reward     the stage-7 reward model, frozen -- what the policy maximises
...
use, and what the stage is called on disk. stage8_instruct_tuning_rlhf.sh is only the
...
def run(ctx):
    ...
    log("=== RLHF (PPO from SFT, reward = stage-7 reward model, KL to frozen SFT) ===")
